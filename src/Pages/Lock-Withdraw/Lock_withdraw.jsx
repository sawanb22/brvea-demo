    ...
    <main className={style.stake_parent + " d-flex justify-content-center"}>
            <div className={style.stake}>
                ...
                <div className={style.top + " mt-4 d-flex justify-content-between align-items-center"}>
                    <div>
                        <p className='opacity-50'>Total Expired Locks</p>
                        <div className='d-flex justify-content-between align-items-center gap-2'>
                            <img src="/assets/demo.svg" alt="" />
                            <p className='mb-0'>{parseFloat?.(
                            convertWeiToEther?.(
                              lockedBalances?.unlockable?.toString()
                            )
                          ).toFixed(2)} </p>
                        </div>
                    </div>
                    <Link to="/lock" className='btn-fill-dark py-3 px-4'>Lock More</Link>
                </div>
            ...
            <img src="/assets/down-arrow.svg" alt="" />
                ...
                <div className={style.inputdiv + " mt-3"}>
                    <div className='d-flex'>
                        <input type="number" value={parseFloat?.(mntBalance?.formatted)
                    ...
                    {/* <div className='mt-3 d-flex align-items-center justify-content-between'>
                        <button>Max</button>
                        <p className='mb-0'><span className='opacity-50'>Balance</span>{parseFloat?.(mntBalance?.formatted)
                            ? parseFloat?.(mntBalance?.formatted).toFixed(2)
                            : "0.0"}</p>
                    </div> */}
                </div>
            ...
            <div className='d-flex gap-2'>
                ...
                {isConnected && address ? (
                      // <Box>
                      <>
                      {parseFloat?.(inputValueWithdraw) > parseFloat?.(lockedBalances?.unlockable?.formatted) ? (
                        <button className='btn-fill-dark py-3 w-100 mt-4'>Insufficient Fund</button>
                      ) : (
                        <button
                          disabled={
                           
                            !withdrawExpiredLocksWaitForTransaction?.isLoading &&
                            !withdrawExpiredLocksContract?.isLoading
                              ? false
                              : true
                          }
                          onClick={async () => {
                            try {
                              await withdrawExpiredLocksContract?.writeAsync();
                            } catch (error) {
                              console.log(error);
                            }
                          }}
                        ...
                        >
                          {(withdrawExpiredLocksWaitForTransaction?.isLoading || withdrawExpiredLocksContract?.isLoading) && (
                            <Loader color="#423A2A" />
                          )}
                          Withdraw Now
                        </button>
