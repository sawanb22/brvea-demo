...
export const Stake_withdraw = () => {
  ...
  const {
    ...
    Abi: POTION_DAO_STAKING_ABI,
    FuncName: "stake",
    Args: [convertEtherToWei?.(inputValue === "" ? "0" : inputValue), false],
    isEnabled:
  ...
  const {
    _useContractWrite: withdrawContract,
    ...
    FuncName: "withdraw",
    Args: [
      convertEtherToWei?.(inputValueWithdraw === "" ? "0" : inputValueWithdraw),
    ],
    isEnabled:
      address !== undefined &&
      parseFloat?.(inputValueWithdraw === "" ? "0" : inputValueWithdraw) > 0 &&
      parseFloat?.(inputValueWithdraw === "" ? "0" : inputValueWithdraw) <=
        parseFloat?.(convertWeiToEther?.(unlockedBalance?.toString())),
  });

  // Function to set value based on percentage
  const handlePercentageClick = (percentage) => {
    if (mntBalance) {
      const balance = parseFloat(mntBalance?.formatted); // Assuming `mntBalance.formatted` is in a readable format
      setInputValue((balance * (percentage / 100)).toFixed(18));
    }
  };
  ...
  const handlePercentageClickWithdraw = (percentage) => {
    if (unlockedBalance) {
      const balance = parseFloat(
        convertWeiToEther(unlockedBalance?.toString())
      );
      setInputValueWithdraw((balance * (percentage / 100)).toFixed(9));
    }
  };
        ...
        <main className={style.stake_parent + " d-flex justify-content-center"}>
            <div className={style.stake}>
                ...
                <div className={style.top + " mt-4 d-flex justify-content-between align-items-center"}>
                    ...
                    <Link to="/stake" className='btn-fill-dark py-3 px-4'>Stake More</Link>
                </div>
                ...
                <div className={style.inputdiv + " mt-3"}>
                    ...
                    <input
                          ...
                          onChange={stackhandleInputChangeWithdraw}
                        />
                    ...
                    <div className='mt-3 d-flex align-items-center justify-content-between'>
                        <button onClick={() => handlePercentageClickWithdraw(100)} >Max</button>
                ...
                <div className={style.bottom + " my-3"}>
                    ...
                    <div className='d-flex justify-content-between '>
                        ...
                        <p className="mb-0">
                          {parseFloat(_rewardDataStake ? getApr() : 0).toFixed(
                            2
                          ) || 0}
                ...
                {/* <button className='btn-fill-dark py-3 w-100 mt-4'>Withdraw Now</button> */}
         
                {isConnected && address ? (
                        <>
                          {parseFloat?.(inputValueWithdraw) >
                          parseFloat?.(unlockedBalance?.formatted) ? (
                            ...
                            <button
                              disabled={
                                parseFloat?.(inputValueWithdraw) > 0 &&
                                !withdrawExpiredLocksWaitForTransaction?.isLoading &&
                                !withdrawContract?.isLoading
                                  ? false
                                  : true
                              }
                              onClick={async () => {
                                try {
                                  await withdrawContract?.writeAsync();
                                } catch (error) {
                                  console.log(error);
                                }
                              }}
