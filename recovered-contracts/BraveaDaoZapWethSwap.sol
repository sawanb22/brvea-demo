...
*/
contract BraveaDaoZapWethSwap is Ownable, ReentrancyGuard {
    ...
    function zap(
        ...
        swap(_lp, _ethIn, address(_info.token));

        // // STEP 2: Add liquditiy
        uint256 _ethAmtToAddLP = IERC20(_info.token0).balanceOf(address(this));
        uint256 _tokenAmtToAddLP = IERC20(_info.token).balanceOf(address(this));
    ...
    function swap(
        ...
        uint256 _ethIn,
        address _token
    ) public  returns (uint256 _ethAmtToSwap, uint256 _tokenAmtReceived) {
    ...
    function doSwapETH(
        ...
        address _toToken,
        uint256 _ethAmt
    ) public  returns (uint256 _tokenReceived) {
    ...
    function calculateSwapInAmount(uint256 _reserveIn, uint256 _tokenIn) public  pure returns (uint256) {
        uint256 swapFee = 17;
