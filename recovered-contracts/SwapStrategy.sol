/**
 *Submitted for verification at testnet.bscscan.com on 2024-09-15
*/
...
library WethUtils {
    ...
    IWETH public constant weth = IWETH(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2); //WETH(WBNB)
    IERC20 public constant _WETH = IERC20(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2); //_WETH  token
...
*/
contract SwapStrategySHLD is ISwapStrategy, Ownable {
    using SafeERC20 for IERC20;
    ...
    function execute(uint256 _wethIn, uint256 _yTokenOut) external override {
        WethUtils._WETH.safeTransferFrom(msg.sender, address(this), _wethIn);
    ...
    function swap(uint256 _wethToSwap, uint256 _minYTokenOut) internal returns (uint256) {
        WethUtils._WETH.safeIncreaseAllowance(address(swapRouter), _wethToSwap);
    ...
    function addLiquidity(
        ...
        if (yTokenAmt > 0 && wethAmt > 0) {
            ...
            WethUtils._WETH.safeIncreaseAllowance(address(swapRouter), wethAmt);
            (uint256 _amountA, uint256 _amountB, uint256 _liquidity) = swapRouter.addLiquidity(
                address(yToken),
                address(WethUtils._WETH),
                yTokenAmt,
                wethAmt,
                _minYTokenOut,
                _minWethOut,
    ...
    function cleanDust() external onlyOwner {
        ...
        WethUtils._WETH.safeTransfer(treasury, WethUtils._WETH.balanceOf(address(this)));
    }
