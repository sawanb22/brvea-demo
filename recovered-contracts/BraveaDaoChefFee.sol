...
// File contracts/interfaces/IPotionDaoStaking.sol

pragma solidity 0.8.4;

interface IPotionDaoStaking {
...
interface IPointSystem {
    function allocatePoint(address _user, uint _point) external;
    function deAllocatePoint(address _user, uint _point) external;
}
...
contract BraveaDaoChefFee is Ownable { 
    ...
    IPotionDaoStaking public rewardMinter;
    ...
    uint256 private constant MAX_REWARD_PER_SECOND = 5000000e18; // 5m token per second
    ...
    mapping(uint => uint) public totalPenaltyFeesCollected;
    uint public penaltyFee = 5000;
    uint public constant BASIS_POINT = 10000;

    IBravea public bravea;
    IPointSystem public pointsystem;
 
    constructor(address initialOwner, address _feeWallet, address _bravea, address _pointSystem) {
        transferOwnership(initialOwner);
        feeWallet = _feeWallet;
        bravea = IBravea(_bravea);
    ...
    function updatePenaltyFees(uint _pid, uint _newfee) external onlyOwner {
        require(_newfee <= 5000, "cann't exceed more than 50%");
        massUpdatePools();
        PoolInfo storage _pool = poolInfo[_pid];
        _pool.fee = _newfee;
    }

    function updateLockingPeriod(uint _pid, uint _newPeriod) external onlyOwner {
        massUpdatePools();
        PoolInfo storage _pool = poolInfo[_pid];
        _pool.lockingPeriod = _newPeriod;
    }   
    ...
    function deposit(
        ...
        user.unlockTime = block.timestamp + pool.lockingPeriod;
    ...
    function withdraw(
        ...
        uint _unlockTime = user.unlockTime;
        ...
        if (_unlockTime > block.timestamp) {
            uint penaltyAmount = (amount * pool.fee) / BASIS_POINT;
            lpToken[pid].safeTransfer(feeWallet, penaltyAmount);
            totalPenaltyFeesCollected[pid] += penaltyAmount;
    ...
    function harvest(uint256 pid, address to) public {
        ...
        if (_pendingReward != 0) {
            if (bravea.tradingActive()) {
                ...
                pointsystem.allocatePoint(to, _pendingReward);
                uint256 boost = (_pendingReward * getBoost(msg.sender, pid)) / nftBoostRate;
                if (boost > 0) pointsystem.allocatePoint(to, boost);
            }
        }
    ...
    function add(
        ...
        require(_fee <= 5000, "cann't exceed more than 50%");
        ...
        poolInfo.push(PoolInfo({allocPoint: allocPoint, lastRewardTime: block.timestamp, lockingPeriod: _lockingPeriod, fee: _fee, accRewardPerShare: 0}));
    ...
    function setRewardPerSecond(uint256 _rewardPerSecond) public onlyOwner {
        require(_rewardPerSecond <= MAX_REWARD_PER_SECOND, "PotionDaoChef::setRewardPerSecond: > MAX_REWARD_PER_SECOND");
        massUpdatePools();
    ...
    function setRewardMinter(IPotionDaoStaking _rewardMinter) external {
        require(address(rewardMinter) == address(0), "PotionDaoChef::setRewardMinter: Cannot redefine rewardMinter");
